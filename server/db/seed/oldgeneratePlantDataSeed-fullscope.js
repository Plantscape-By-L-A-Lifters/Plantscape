// TO RUN: node generateSeed.js
const fs = require("fs"); // For synchronous functions like writeFileSync
const fsPromises = require("fs").promises; // For promise-based functions like readFile
...
// --- START CSV Data (Now dynamic, paths for files) ---
...
async function generateSeedFile() {
  ...
  stylesData.forEach((style) => {
    const name = style.style.trim().toLowerCase();
  ...
  defaultStylesFromContext.forEach((contextStyle) => {
    const name = contextStyle.name.trim().toLowerCase();
    if (name && !allDesignNames.has(name)) {
      allDesignNames.add(name);
      const id = uuidv4();
      designStylesMap.set(name, id);
  ...
  plantsData.forEach((plant) => {
    ...
    plantInserts.push({
      ...
      other_common_names: otherCommonNames,
      technical_name: technicalName,
      growth_form: growthForm,
      is_toxic: isToxic,
      sun_requirements: sunRequirements,
      height_min_ft: heightMin,
      height_max_ft: heightMax,
      width_min_ft: widthMin,
      width_max_ft: widthMax,
